...
<runner> spatial render --session <id> --revision <n> --box hands-reference --views front,left,top --pass color --output .human2ai-data/output/hands-three-views.png
```
...
Use `--views front,left,top` to combine only the directions needed for the task into one PNG. It requires `--box` and accepts 1–6 distinct names from `front,back,left,right,top,bottom`, separated by commas; whitespace around names is ignored. The supplied order is retained, left to right and then top to bottom, with at most three columns. A single selected direction still includes its label; use `--view front` for the original unlabelled square image. `--view` and `--views` are mutually exclusive; `sheet`, duplicate names and empty selections are invalid in `--views`. All four render passes support combinations. Export reads one saved revision and does not change the box, scene or browser's active direction buttons. The browser and CLI share the sheet layout rules; browser captions follow its locale while CLI captions retain English box-local direction/axis labels. Choose the useful views yourself, or use the user's explicitly requested directions; no browser interaction is required. Create the output directory before exporting.

The HTTP route is `GET /api/v1/sessions/:sessionId/spatial/camera-boxes/:boxId.png?revision=<n>&view=sheet&pass=color`. For a combination, replace `view=sheet` with `views=front,left,top`; the response's `x-spatial-box-views` header lists the rendered directions in order, and caching distinguishes both selection and order. Render is read-only and revision-addressed; box edits are saved and undoable through the existing draft history. `session connect` discovers `spatial.render-box@1` and `spatial.render-box-views@1`; `spatial methods` exposes the closed operation schemas, `cameraBoxViews`, the `cameraBoxViewSelection` parameter contract, and `cameraBoxGuidance` (canonical wording: `spatial.cameraBoxGenerationGuidance` in both locale resources). The CLI result includes `views` for an explicit combination and retains `view` for the original single-view/six-view mode.

When generating, export the main camera and auxiliary boxes from the **same revision**. Include the returned `cameraBoxGuidance` in the prompt: the main camera determines composition; the sheet shows the same subjects at one moment from the selected directions, solely to resolve occlusion, digit ownership and approximate flexion. Request one final image, with no contact-sheet layout, duplicate subjects, direction labels, or joint/body markings. Preserve gesture intent while allowing natural anatomical corrections. Inspect the exported views yourself before generation; start with the main camera plus the useful hand views, adding full-space depth or an informative full-resolution face when useful. More views do not guarantee correct fingers, and cannot repair an impossible underlying pose automatically.
